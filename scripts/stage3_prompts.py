...
shards live under data/prompt/stage3/ and are committed to GitHub so a
downstream OpenAI Batch job (image-gen-2) can render them later.
...
images/neighborhoods_dynamic/{slug}.png.
...
from neighborhood_hero_pipeline import (  # noqa: E402
    REPO_ROOT, build_prompt, load_manifest,
)
...
def _write_shard(args: tuple[int, list[dict[str, str]]]) -> str:
    shard_no, rows = args
    STAGE3_DIR.mkdir(parents=True, exist_ok=True)
    path = STAGE3_DIR / f"stage3_prompts_shard_{shard_no:02d}.csv"
